<?php
$canManageDisposisi = (bool) $can_manage_disposisi;
$dataDisposisi = $data_disposisi ?? [];
?>
<div class="row">
    <div class="col-lg-12">
        <div class="panel panel-yellow">
            <div class="panel-heading">
                <?= 'Daftar ' . esc($judul) ?>
                <?php if ($canManageDisposisi): ?>
                    &nbsp;&nbsp;
                    <button class="btn btn-success" data-toggle="modal" data-target="#tambah_surat_masuk">
                        <i class="fa fa-envelope"></i> Tambah <?= esc($judul) ?>
                    </button>&nbsp;&nbsp;
                    <a href="<?= base_url('home/disposisi_selesai/' . $surat_id) ?>" class="btn btn-info">
                        <i class="fa fa-check"></i> <?= esc($judul) ?> Selesai
                    </a>
                <?php else: ?>
                    <b style="color: green">(DISPOSISI SELESAI)</b>
                <?php endif; ?>
            </div>
                ...
                <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                    ...
                    <?php foreach ($dataDisposisi as $disposisi): ?>
                            <tr>
                                ...
                                <td class="text-center" style="vertical-align: middle;"><?= esc($disposisi->nama_jabatan) ?></td>
                                <td class="text-center" style="vertical-align: middle;"><?= esc($disposisi->nama_pegawai) ?></td>
                                <td class="text-center" style="vertical-align: middle;"><?= esc($disposisi->tgl_disposisi) ?></td>
                                <td class="text-center" style="vertical-align: middle;"><?= esc($disposisi->keterangan) ?></td>
...
<div class="modal fade" id="tambah_surat_masuk" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form role="form" action="<?= base_url('home/tambah_disposisi/' . $surat_id) ?>" method="post">
                            ...
                            <select class="form-control" name="tujuan_unit" onchange="get_pegawai_id_by_jabatan(this.value)">
                                ...
                                <?php if (isset($drop_down_jabatan)): ?>
                                    <?php foreach ($drop_down_jabatan as $jabatan): ?>
                                        <?php if ((int) $jabatan->id_jabatan !== $current_user_id_jabatan): ?>
